...
contract RewarderMock is IRewarder {
    ...
    function onDefiantReward (uint256, address user, address to, uint256 defiantAmount, uint256) onlyMCV2 override external {
        uint256 pendingReward = defiantAmount.mul(rewardMultiplier) / REWARD_TOKEN_DIVISOR;
    ...
    function pendingTokens(uint256 pid, address user, uint256 defiantAmount) override external view returns (IERC20[] memory rewardTokens, uint256[] memory rewardAmounts) {
        ...
        _rewardAmounts[0] = defiantAmount.mul(rewardMultiplier) / REWARD_TOKEN_DIVISOR;
