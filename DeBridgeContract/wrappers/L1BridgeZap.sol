...
/**
 * @title L1BridgeZap
 * @notice This contract is responsible for handling user Zaps into the defiantBridge contract, through the defiant Swap contracts. It does so
 ...
 * This is done in hopes of automating portions of the bridge user experience to users, while keeping the defiantBridge contract logic small.
 *
 * @dev This contract should be deployed with a base Swap.sol address and a defiantBridge.sol address, otherwise, it will not function.
 */
contract L1BridgeZap {
   ...
   */
  constructor(address payable _wethAddress, ISwap _baseSwap, IDefiantBridge _defiantBridge) public {
  ...
  /**
   * @notice Wraps defiantBridge depositAndSwap() function to make it compatible w/ ETH -> WETH conversions
   * @param to address on other chain to bridge assets to
   * @param chainId which chain to bridge assets onto
   * @param amount Amount in native token decimals to transfer cross-chain pre-fees
  ...
  function depositETHAndSwap(
    ...
    uint256 deadline
      ...
      require(msg.value > 0 && msg.value == amount, 'INCORRECT MSG VALUE');
      IWETH9(WETH_ADDRESS).deposit{value: msg.value}();
      defiantBridge.depositAndSwap(to, chainId, IERC20(WETH_ADDRESS), amount, tokenIndexFrom, tokenIndexTo, minDy, deadline);
    }
  ...
  function zapAndDeposit(
    ...
    if (token.allowance(address(this), address(defiantBridge)) < liqAdded) {
      token.safeApprove(address(defiantBridge), MAX_UINT256);
    }
    defiantBridge.deposit(to, chainId, token, liqAdded);
  }
  ...
  function zapAndDepositAndSwap(
    ...
    /**
   * @notice Wraps defiantBridge deposit() function
   * @param to address on other chain to bridge assets to
   * @param chainId which chain to bridge assets onto
   * @param token ERC20 compatible token to deposit into the bridge
   * @param amount Amount in native token decimals to transfer cross-chain pre-fees
  ...
  /**
   * @notice Wraps defiantBridge depositAndSwap() function
   * @param to address on other chain to bridge assets to
   * @param chainId which chain to bridge assets onto
   * @param token ERC20 compatible token to deposit into the bridge
   * @param amount Amount in native token decimals to transfer cross-chain pre-fees
  ...
  function depositAndSwap(
    ...
    uint256 deadline
      ...
      defiantBridge.depositAndSwap(to, chainId, token, amount, tokenIndexFrom, tokenIndexTo, minDy, deadline);
  }


    /**
   * @notice Wraps defiantBridge redeem() function
   * @param to address on other chain to bridge assets to
   * @param chainId which chain to bridge assets onto
