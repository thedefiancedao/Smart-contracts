...
contract MigratorBridgeZap {
    ...
    IDefiantBridge constant defiantBridge = IDefiantBridge(0xd123f70AE324d34A9E76b67a27bf77593bA8749f);
    ...
    constructor() public {
        ...
        newToken.safeApprove(address(defiantBridge), MAX_UINT256);
    }
    ...
    function migrateAndBridge(uint256 amount, address to, uint256 chainId) external {
        legacyToken.safeTransferFrom(msg.sender, address(this), amount);
        erc20Migrator.migrate(amount);
        defiantBridge.redeem(to, chainId, newToken, amount.mul(5).div(2));
    }
}