...
const DeStaking = artifacts.require('DeStaking');
const DeStakingMock = artifacts.require('DeStakingMock');
...
contract('DeStaking', accounts => {
  ...
  function initialize(...params) {
    ...
    return DeStaking.methods[initializeMethod](...params, { from: owner });
  }
  ...
  beforeEach(async () => {
    ...
    DeStaking = await DeStaking.new();
    liquidityProvidersRewardContract = await ReceiverMock.new();
    await initialize();
    await stakeToken.initialize('Stake', 'STAKE', 18, 0, owner, [owner, DeStaking.address], [], DeStaking.address);
  });

  describe('initialize', () => {
    it('should be set up correctly', async () => {
      expect(await DeStaking.token()).to.equal(stakeToken.address);
      const params = await DeStaking.getSigmoidParameters();
  ...
  function testDeposit(directly) {
    beforeEach(async () => {
      ...
      await stakeToken.approve(DeStaking.address, ether('10000'), { from: user1 });
    });
    it('should deposit', async () => {
      const value = ether('100');
      let receipt;
      if (directly) {
        receipt = await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
        ...
        receipt = await stakeToken.transfer(DeStaking.address, value, { from: user1 });
      }
      const timestamp = await getBlockTimestamp(receipt);
      expect(await DeStaking.balances(user1, 1)).to.be.bignumber.equal(value);
      expect(await DeStaking.depositDates(user1, 1)).to.be.bignumber.equal(timestamp);
    });
    it('should accrue emission', async () => {
      ...
      const totalStaked = await DeStaking.totalStaked();
      receipt = await DeStaking.methods['deposit(uint256,uint256)'](1, value, { from: user1 });
      ...
      expect(await DeStaking.balances(user1, 1)).to.be.bignumber.equal(value.add(value).add(userAccruedEmission));
      expect(await DeStaking.depositDates(user1, 1)).to.be.bignumber.equal(timestampAfter);
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(value.add(value).add(userAccruedEmission));
    });
    it('should deposit using an old id', async () => {
      await DeStaking.setFee(0, { from: owner });
      const value = ether('100');
      if (directly) {
        await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
        await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
      } else {
        await stakeToken.transfer(DeStaking.address, value, { from: user1 });
        await stakeToken.transfer(DeStaking.address, value, { from: user1 });
      }
      await time.increase(YEAR);
      await DeStaking.makeForcedWithdrawal(1, 0, { from: user1 });
      await DeStaking.makeForcedWithdrawal(2, 0, { from: user1 });
      expect(await DeStaking.balances(user1, 1)).to.be.bignumber.equal(new BN(0));
      expect(await DeStaking.balances(user1, 2)).to.be.bignumber.equal(new BN(0));
      expect(await DeStaking.depositDates(user1, 1)).to.be.bignumber.equal(new BN(0));
      expect(await DeStaking.depositDates(user1, 2)).to.be.bignumber.equal(new BN(0));

      let receipt = await DeStaking.methods['deposit(uint256,uint256)'](1, value, { from: user1 });
      expect(await DeStaking.balances(user1, 1)).to.be.bignumber.equal(value);
      const timestampBefore = await getBlockTimestamp(receipt);
      const totalSupply = await stakeToken.totalSupply();
      const totalStaked = await DeStaking.totalStaked();
      ...
      await time.increase(YEAR);
      receipt = await DeStaking.makeForcedWithdrawal(1, 0, { from: user1 });
      const timestampAfter = await getBlockTimestamp(receipt);
      const timePassed = timestampAfter.sub(timestampBefore);
      const userAccruedEmission = calculateUserAccruedEmission(value, timePassed, totalSupply, totalStaked);
    ...
    it('fails if deposit value is zero', async () => {
      if (directly) {
        await expectRevert(
          DeStaking.methods['deposit(uint256)'](0, { from: user1 }),
          'deposit amount should be more than 0'
        );
      } else {
        await expectRevert(
          stakeToken.transfer(DeStaking.address, 0, { from: user1 }),
          `you can't transfer to bridge contract` // if onTokenTransfer() fails
        );
      }
    });
    it('fails if emission is stopped', async () => {
      await DeStaking.setSigmoidParameters(0, sigmoidParamB, sigmoidParamC);
      await DeStaking.setTotalSupplyFactor(0);
      await time.increase(PARAM_UPDATE_DELAY.add(new BN(1)));
      if (directly) {
        await expectRevert(
          DeStaking.methods['deposit(uint256)'](ether('1'), { from: user1 }),
          'emission stopped'
        );
      } else {
        await expectRevert(
          stakeToken.transfer(DeStaking.address, ether('1'), { from: user1 }),
          `you can't transfer to bridge contract` // if onTokenTransfer() fails
        );
      }
    });
  ...
  describe('deposit', () => {
    ...
    it('fails if wrong deposit id', async () => {
      await expectRevert(
        DeStaking.methods['deposit(uint256,uint256)'](1, ether('100'), { from: user1 }),
        'wrong deposit id'
      );
    });
  });
  describe('onTokenTransfer', () => {
    ...
    it('fails if not a token address', async () => {
      await expectRevert(
        DeStaking.onTokenTransfer(user1, ether('1'), '0x', { from: owner }),
  ...
  describe('makeForcedWithdrawal', () => {
    ...
    it('should withdraw', async () => { 
      let receipt = await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
      let timestampBefore = await getBlockTimestamp(receipt);
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(value);
      let totalSupply = await stakeToken.totalSupply();
      let totalStaked = await DeStaking.totalStaked();
      await time.increase(1); // make sure the timestamp of two consequent blocks is different
      receipt = await DeStaking.makeForcedWithdrawal(1, oneEther, { from: user1 });
      ...
      expect(await DeStaking.balances(user1, 1)).to.be.bignumber.equal(value.sub(oneEther));
      expect(await DeStaking.depositDates(user1, 1)).to.be.bignumber.equal(timestampBefore);
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(value.sub(oneEther));
      ...
      totalStaked = await DeStaking.totalStaked();
      await time.increase(1); // make sure the timestamp of two consequent blocks is different
      receipt = await DeStaking.makeForcedWithdrawal(1, 0, { from: user1 });
      ...
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(new BN(0));
      expect(await DeStaking.depositDates(user1, 1)).to.be.bignumber.equal(new BN(0));
      expectEvent(receipt, 'Withdrawn', {
        sender: user1,
    ...
    it('should accrue emission for different users from 1 address', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await DeStaking.methods['deposit(uint256)'](values[i], { from: exchange });
        expect(await DeStaking.balances(exchange, i + 1)).to.be.bignumber.equal(values[i]);
      }
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(values.reduce((acc, cur) => acc.add(cur), new BN(0)));
      let exchangeBalance = await stakeToken.balanceOf(exchange);
      await time.increase(MONTH * 4);
      for (let i = 0; i < 3; i++) {
        const timestampBefore = await DeStaking.depositDates(exchange, i + 1);
        const totalSupply = await stakeToken.totalSupply();
        const totalStaked = await DeStaking.totalStaked();
        const receipt = await DeStaking.makeForcedWithdrawal(i + 1, 0, { from: user1 });
        const timestampAfter = await getBlockTimestamp(receipt);
        const timePassed = timestampAfter.sub(timestampBefore);
        ...
        expect(userAccruedEmission).to.be.bignumber.gt(new BN(0));
        expect(feeValue).to.be.bignumber.gt(new BN(0));
        expect(await DeStaking.balances(exchange, i + 1)).to.be.bignumber.equal(new BN(0));
    ...
    it('should pause minting tokens', async () => {
      await DeStaking.setFee(0, { from: owner });
      await DeStaking.methods['deposit(uint256)'](ether('100'), { from: user1 });
      await DeStaking.methods['deposit(uint256)'](ether('100'), { from: user1 });
      await time.increase(YEAR);
      ...
      expect(await stakeToken.isMinter(DeStaking.address)).to.be.equal(true);
      await stakeToken.removeMinter(DeStaking.address);
      expect(await stakeToken.isMinter(DeStaking.address)).to.be.equal(false);
      await DeStaking.setSigmoidParameters(0, sigmoidParamB, sigmoidParamC, { from: owner });
      await DeStaking.setTotalSupplyFactor(0, { from: owner });
      await time.increase(PARAM_UPDATE_DELAY.add(new BN(1)));
    ...
    it('fails if trying to withdraw more than deposited', async () => {
      await DeStaking.methods['deposit(uint256)'](ether('10'), { from: user1 });
      await time.increase(YEAR);
      await expectRevert(
        DeStaking.makeForcedWithdrawal(1, ether('10.000000000000000001'), { from: user1 }),
        'insufficient funds'
      );
      await DeStaking.makeForcedWithdrawal(1, ether('10'), { from: user1 });
    });
    it('fails if wrong deposit id', async () => {
      await DeStaking.methods['deposit(uint256)'](ether('10'), { from: user1 });
      await expectRevert(
        DeStaking.makeForcedWithdrawal(2, ether('10'), { from: user1 }),
        'wrong deposit id'
      );
      await DeStaking.makeForcedWithdrawal(1, ether('10'), { from: user1 });
    });
    it('fails if zero balance', async () => {
      ...
      await DeStaking.makeForcedWithdrawal(1, ether('10'), { from: user1 });
      await expectRevert(
        DeStaking.makeForcedWithdrawal(1, ether('10'), { from: user1 }),
        'insufficient funds'
      );
    });
    it('should withdraw entire deposit by several parts', async () => {
      await DeStaking.setFee(0);
      const depositValue = ether('1000');
      let receipt = await DeStaking.methods['deposit(uint256)'](depositValue, { from: user1 });
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        const totalSupply = await stakeToken.totalSupply();
        const totalStaked = await DeStaking.totalStaked();
        receipt = await DeStaking.makeForcedWithdrawal(1, parts[i], { from: user1 });
        const timestampAfter = await getBlockTimestamp(receipt);
        const timePassed = timestampAfter.sub(timestampBefore);
    ...
    it('should withdraw the same amount', async () => {
      ...
      await stakeToken.approve(DeStaking.address, ether('10000'), { from: user2 });

      receipt = await DeStaking.methods['deposit(uint256)'](value, { from: user2 });
      timestampBefore = await getBlockTimestamp(receipt);
      expect(await DeStaking.balances(user2, 1)).to.be.bignumber.equal(value);
      await time.increase(YEAR);
      totalSupply = await stakeToken.totalSupply();
      totalStaked = await DeStaking.totalStaked();
      receipt = await DeStaking.makeForcedWithdrawal(1, value, { from: user2 });
      timestampAfter = await getBlockTimestamp(receipt);
      timePassed = timestampAfter.sub(timestampBefore);
      userAccruedEmission = calculateUserAccruedEmission(value, timePassed, totalSupply, totalStaked);
      expect(await DeStaking.balances(user2, 1)).to.be.bignumber.equal(new BN(0));
  ...
  describe('requestWithdrawal', () => {
    it('should request', async () => {
      ...
      await stakeToken.mint(user1, value, { from: owner });
      await stakeToken.transfer(DeStaking.address, value, { from: user1 });
      const receipt = await DeStaking.requestWithdrawal(1, { from: user1 });
      const timestamp = await getBlockTimestamp(receipt);
      expect(await DeStaking.withdrawalRequestsDates(user1, 1)).to.be.bignumber.equal(timestamp);
    ...
    it('fails if wrong deposit id', async () => {
      await expectRevert(DeStaking.requestWithdrawal(1, { from: user1 }), 'wrong deposit id');
    });
  });
  describe('makeRequestedWithdrawal', () => {
    ...
    it('should withdraw', async () => {
      let receipt = await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
      const timestampBefore = await getBlockTimestamp(receipt);
      await DeStaking.requestWithdrawal(1, { from: user1 });
      await time.increase(withdrawalLockDuration);
      const totalSupply = await stakeToken.totalSupply();
      const totalStaked = await DeStaking.totalStaked();
      receipt = await DeStaking.makeRequestedWithdrawal(1, 0, { from: user1 });
      const timestampAfter = await getBlockTimestamp(receipt);
      const timePassed = timestampAfter.sub(timestampBefore);
      const userAccruedEmission = calculateUserAccruedEmission(value, timePassed, totalSupply, totalStaked);
      expect(await DeStaking.withdrawalRequestsDates(user1, 1)).to.be.bignumber.equal(new BN(0));
      expect(await DeStaking.balances(user1, 1)).to.be.bignumber.equal(new BN(0));
      expect(await stakeToken.balanceOf(user1)).to.be.bignumber.equal(value.add(userAccruedEmission));
      expectEvent(receipt, 'Withdrawn', {
        sender: user1,
    ...
    it('should fail if not requested', async () => {
      await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
      await expectRevert(DeStaking.makeRequestedWithdrawal(1, 0, { from: user1 }), `withdrawal wasn't requested`);
    });
    it('should fail if too early', async () => {
      await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
      await DeStaking.requestWithdrawal(1, { from: user1 });
      await time.increase(withdrawalLockDuration.sub(new BN(5)));
      await expectRevert(DeStaking.makeRequestedWithdrawal(1, 0, { from: user1 }), 'too early');
    });
    it('should fail if too late', async () => {
      await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
      await DeStaking.requestWithdrawal(1, { from: user1 });
      await time.increase(withdrawalLockDuration.add(withdrawalUnlockDuration).add(new BN(1)));
      await expectRevert(DeStaking.makeRequestedWithdrawal(1, 0, { from: user1 }), 'too late');
    });
  });
  describe('totalStaked', () => {
    it('should be calculated correctly', async () => {
      ...
      await stakeToken.mint(user1, value, { from: owner });
      await stakeToken.mint(user2, value, { from: owner });
      await stakeToken.transfer(DeStaking.address, ether('10'), { from: owner });
      expectedTotalStaked = expectedTotalStaked.add(ether('10'));
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(expectedTotalStaked);
      await time.increase(1);
      let receipt = await stakeToken.transfer(DeStaking.address, ether('500'), { from: user1 });
      const timestampBefore = await getBlockTimestamp(receipt);
      expectedTotalStaked = expectedTotalStaked.add(ether('500'));
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(expectedTotalStaked);
      await time.increase(1);
      await DeStaking.makeForcedWithdrawal(1, ether('250'), { from: user1 });
      expectedTotalStaked = expectedTotalStaked.sub(ether('250'));
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(expectedTotalStaked);
      await time.increase(1);
      await stakeToken.transfer(DeStaking.address, ether('1000'), { from: user2 });
      expectedTotalStaked = expectedTotalStaked.add(ether('1000'));
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(expectedTotalStaked);
      await time.increase(1);
      await stakeToken.transfer(DeStaking.address, ether('40'), { from: user1 });
      expectedTotalStaked = expectedTotalStaked.add(ether('40'));
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(expectedTotalStaked);
      await time.increase(1);
      const totalSupply = await stakeToken.totalSupply();
      const totalStaked = await DeStaking.totalStaked();
      await stakeToken.approve(DeStaking.address, ether('333'), { from: user1 });
      receipt = await DeStaking.methods['deposit(uint256,uint256)'](1, ether('333'), { from: user1 });
      const timestampAfter = await getBlockTimestamp(receipt);
      const timePassed = timestampAfter.sub(timestampBefore);
      ...
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(expectedTotalStaked);
      await time.increase(1);
      await DeStaking.makeForcedWithdrawal(1, 0, { from: owner });
      expectedTotalStaked = expectedTotalStaked.sub(ether('10'));
      expect(await DeStaking.totalStaked()).to.be.bignumber.equal(expectedTotalStaked);
      await time.increase(1);
      await DeStaking.makeForcedWithdrawal(1, 0, { from: user2 });
  ...
  describe('setFee', () => {
    it('should set', async () => {
      ...
      expect(await DeStaking.fee()).to.be.bignumber.equal(fee);
      expect(newFee).to.be.bignumber.not.equal(fee);
      const receipt = await DeStaking.setFee(newFee, { from: owner });
      expectEvent(receipt, 'FeeSet', { value: newFee, sender: owner });
      await time.increase(PARAM_UPDATE_DELAY.sub(new BN(1)));
      expect(await DeStaking.fee()).to.be.bignumber.equal(fee);
      await time.increase(2);
      expect(await DeStaking.fee()).to.be.bignumber.equal(newFee);
    });
    it('fails if not an owner', async () => {
      await expectRevert(
        DeStaking.setFee(ether('0.1'), { from: user1 }),
        'Ownable: caller is not the owner',
      );
    });
    it('fails if greater than 1 ether', async () => {
      await expectRevert(DeStaking.setFee(ether('1.01'), { from: owner }), 'should be less than or equal to 1 ether');
    });
  });
  describe('setWithdrawalLockDuration', () => {
    it('should set', async () => {
      const newWithdrawalLockDuration = new BN(1000);
      expect(await DeStaking.withdrawalLockDuration()).to.be.bignumber.equal(withdrawalLockDuration);
      expect(newWithdrawalLockDuration).to.be.bignumber.not.equal(withdrawalLockDuration);
      const receipt = await DeStaking.setWithdrawalLockDuration(newWithdrawalLockDuration, { from: owner });
      expectEvent(receipt, 'WithdrawalLockDurationSet', { value: newWithdrawalLockDuration, sender: owner });
      await time.increase(PARAM_UPDATE_DELAY.sub(new BN(1)));
      expect(await DeStaking.withdrawalLockDuration()).to.be.bignumber.equal(withdrawalLockDuration);
      await time.increase(2);
      expect(await DeStaking.withdrawalLockDuration()).to.be.bignumber.equal(newWithdrawalLockDuration);
    });
    it('fails if not an owner', async () => {
      await expectRevert(
        DeStaking.setWithdrawalLockDuration(new BN(1000), { from: user1 }),
        'Ownable: caller is not the owner',
      );
    });
    it('fails if greater than 30 days', async () => {
      await expectRevert(DeStaking.setWithdrawalLockDuration(2592001, { from: owner }), `shouldn't be greater than 30 days`);
    });
  });
  describe('setWithdrawalUnlockDuration', () => {
    it('should set', async () => {
      const newWithdrawalUnlockDuration = new BN(10000);
      expect(await DeStaking.withdrawalUnlockDuration()).to.be.bignumber.equal(withdrawalUnlockDuration);
      expect(newWithdrawalUnlockDuration).to.be.bignumber.not.equal(withdrawalUnlockDuration);
      const receipt = await DeStaking.setWithdrawalUnlockDuration(newWithdrawalUnlockDuration, { from: owner });
      expectEvent(receipt, 'WithdrawalUnlockDurationSet', { value: newWithdrawalUnlockDuration, sender: owner });
      await time.increase(PARAM_UPDATE_DELAY.sub(new BN(1)));
      expect(await DeStaking.withdrawalUnlockDuration()).to.be.bignumber.equal(withdrawalUnlockDuration);
      await time.increase(2);
      expect(await DeStaking.withdrawalUnlockDuration()).to.be.bignumber.equal(newWithdrawalUnlockDuration);
    });
    it('fails if not an owner', async () => {
      await expectRevert(
        DeStaking.setWithdrawalUnlockDuration(new BN(10000), { from: user1 }),
        'Ownable: caller is not the owner',
      );
    });
    it('fails if less than 1 hour', async () => {
      await expectRevert(DeStaking.setWithdrawalUnlockDuration(3599, { from: owner }), `shouldn't be less than 1 hour`);
    });
  });
  describe('setTotalSupplyFactor', () => {
    it('should set', async () => {
      const newTotalSupplyFactor = ether('0.9');
      expect(await DeStaking.totalSupplyFactor()).to.be.bignumber.equal(totalSupplyFactor);
      expect(newTotalSupplyFactor).to.be.bignumber.not.equal(totalSupplyFactor);
      const receipt = await DeStaking.setTotalSupplyFactor(newTotalSupplyFactor, { from: owner });
      expectEvent(receipt, 'TotalSupplyFactorSet', { value: newTotalSupplyFactor, sender: owner });
      await time.increase(PARAM_UPDATE_DELAY.sub(new BN(1)));
      expect(await DeStaking.totalSupplyFactor()).to.be.bignumber.equal(totalSupplyFactor);
      await time.increase(2);
      expect(await DeStaking.totalSupplyFactor()).to.be.bignumber.equal(newTotalSupplyFactor);
    });
    it('fails if not an owner', async () => {
      await expectRevert(
        DeStaking.setTotalSupplyFactor(ether('0.9'), { from: user1 }),
        'Ownable: caller is not the owner',
      );
    });
    it('fails if greater than 1 ether', async () => {
      await expectRevert(
        DeStaking.setTotalSupplyFactor(ether('1.01'), { from: owner }),
        'should be less than or equal to 1 ether'
      );
    });
  });
  describe('setSigmoidParameters', () => {
    it('should set', async () => {
      ...
      let sigmoidParams = await DeStaking.getSigmoidParameters();
      ...
      const receipt = await DeStaking.setSigmoidParameters(newSigmoidParams.a, newSigmoidParams.b, newSigmoidParams.c, { from: owner });
      expectEvent(receipt, 'SigmoidParametersSet', { a: newSigmoidParams.a, b: newSigmoidParams.b, c: newSigmoidParams.c, sender: owner });
      await time.increase(PARAM_UPDATE_DELAY.sub(new BN(1)));
      sigmoidParams = await DeStaking.getSigmoidParameters();
      expect(sigmoidParams.a).to.be.bignumber.equal(sigmoidParamA);
      expect(sigmoidParams.b).to.be.bignumber.equal(sigmoidParamB);
      expect(sigmoidParams.c).to.be.bignumber.equal(sigmoidParamC);
      await time.increase(2);
    ...
    it('fails if not an owner', async () => {
      const newSigmoidParams = { a: ether('0.065'), b: new BN(10000), c: new BN(999999) };
      await expectRevert(
        DeStaking.setSigmoidParameters(newSigmoidParams.a, newSigmoidParams.b, newSigmoidParams.c, { from: user1 }),
        'Ownable: caller is not the owner',
      );
    });
    it('fails if wrong values', async () => {
      await expectRevert(
        DeStaking.setSigmoidParameters(ether('0.076'), sigmoidParamB, sigmoidParamC),
        'should be less than or equal to a half of the maximum emission rate'
      );
      await expectRevert(DeStaking.setSigmoidParameters(sigmoidParamA, sigmoidParamB, 0), 'should be greater than 0');
    });
  });
  describe('setLiquidityProvidersRewardAddress', () => {
    it('should set', async () => {
      const newLiquidityProvidersRewardAddress = user1;
      expect(await DeStaking.liquidityProvidersRewardAddress()).to.be.bignumber.equal(liquidityProvidersRewardAddress);
      expect(newLiquidityProvidersRewardAddress).to.be.bignumber.not.equal(liquidityProvidersRewardAddress);
      const receipt = await DeStaking.setLiquidityProvidersRewardAddress(newLiquidityProvidersRewardAddress, { from: owner });
      expectEvent(receipt, 'LiquidityProvidersRewardAddressSet', { value: newLiquidityProvidersRewardAddress, sender: owner });
      await time.increase(PARAM_UPDATE_DELAY.sub(new BN(1)));
      expect(await DeStaking.liquidityProvidersRewardAddress()).to.be.bignumber.equal(liquidityProvidersRewardAddress);
      await time.increase(2);
      expect(await DeStaking.liquidityProvidersRewardAddress()).to.be.bignumber.equal(newLiquidityProvidersRewardAddress);
    });
    it('fails if not an owner', async () => {
      await expectRevert(
        DeStaking.setLiquidityProvidersRewardAddress(user1, { from: user1 }),
        'Ownable: caller is not the owner',
      );
    });
    it('fails if equal to zero', async () => {
      await expectRevert(
        DeStaking.setLiquidityProvidersRewardAddress(constants.ZERO_ADDRESS, { from: owner }),
        'zero address'
      );
    });
    it('fails if equal to the address of DeStaking contract', async () => {
      await expectRevert(
        DeStaking.setLiquidityProvidersRewardAddress(DeStaking.address, { from: owner }),
        'wrong address'
      );
    });
  });
  describe('claimTokens', () => {
    it('should claim tokens', async () => {
      ...
      await anotherToken.transfer(DeStaking.address, value, { from: user1 });
      expect(await anotherToken.balanceOf(DeStaking.address)).to.be.bignumber.equal(value);
      expect(await anotherToken.balanceOf(owner)).to.be.bignumber.equal(new BN(0));
      await DeStaking.claimTokens(anotherToken.address, owner, value, { from: owner });
      expect(await anotherToken.balanceOf(DeStaking.address)).to.be.bignumber.equal(new BN(0));
      expect(await anotherToken.balanceOf(owner)).to.be.bignumber.equal(value);
    });
    it('should claim STAKE tokens', async () => {
      await stakeToken.mint(DeStaking.address, ether('10'), { from: owner });
      await stakeToken.mint(user1, ether('100'), { from: owner });
      expect(await stakeToken.balanceOf(DeStaking.address)).to.be.bignumber.equal(ether('10'));
      expect(await stakeToken.balanceOf(user1)).to.be.bignumber.equal(ether('100'));
      await stakeToken.transfer(DeStaking.address, ether('100'), { from: user1 });
      expect(await stakeToken.balanceOf(DeStaking.address)).to.be.bignumber.equal(ether('110'));
      expect(await stakeToken.balanceOf(owner)).to.be.bignumber.equal(new BN(0));
      expect(await DeStaking.balances(user1, 1)).to.be.bignumber.equal(ether('100'));
      await DeStaking.claimTokens(stakeToken.address, owner, ether('10'), { from: owner });
      expect(await stakeToken.balanceOf(DeStaking.address)).to.be.bignumber.equal(ether('100'));
      expect(await stakeToken.balanceOf(owner)).to.be.bignumber.equal(ether('10'));
      expect(await DeStaking.balances(user1, 1)).to.be.bignumber.equal(ether('100'));
      await expectRevert(DeStaking.claimTokens(stakeToken.address, owner, ether('10'), { from: owner }), 'insufficient funds');
    });
    async function claimEtherAndSend(to) {
      DeStaking = await DeStakingMock.new();
      await initialize();
      const value = ether('10');
      expect(await balance.current(DeStaking.address)).to.be.bignumber.equal(new BN(0));
      await send.ether(user1, DeStaking.address, value);
      expect(await balance.current(DeStaking.address)).to.be.bignumber.equal(value);
      const balanceBefore = await balance.current(to);
      await DeStaking.claimTokens(constants.ZERO_ADDRESS, to, value, { from: owner, gasPrice: 0 });
      expect(await balance.current(DeStaking.address)).to.be.bignumber.equal(new BN(0));
    ...
    it('fails if not an owner', async () => {
      await expectRevert(
        DeStaking.claimTokens(constants.ZERO_ADDRESS, owner, ether('1'), { from: user1 }),
        'Ownable: caller is not the owner',
      );
    });
    it('fails if invalid recipient', async () => {
      await expectRevert(
        DeStaking.claimTokens(constants.ZERO_ADDRESS, constants.ZERO_ADDRESS, ether('1'), { from: owner }),
        'not a valid recipient',
      );
      await expectRevert(
        DeStaking.claimTokens(constants.ZERO_ADDRESS, DeStaking.address, ether('1'), { from: owner }),
        'not a valid recipient',
      );
    });
    it('fails if zero amount', async () => {
      await expectRevert(
        DeStaking.claimTokens(constants.ZERO_ADDRESS, owner, 0, { from: owner }),
  ...
  describe('getSupplyBasedEmissionRate', () => {
    it('should be calculated correctly', async () => {
      ...
      await stakeToken.approve(DeStaking.address, value, { from: user1 });
      await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
      const totalStaked = value;
      const supplyBasedEmissionRate1 = calculateSupplyBasedEmissionRate(totalSupply, totalStaked);
      expect(await DeStaking.getSupplyBasedEmissionRate()).to.be.bignumber.equal(supplyBasedEmissionRate1);
      const newTotalSupplyFactor = ether('0.7');
      await DeStaking.setTotalSupplyFactor(newTotalSupplyFactor);
      const supplyBasedEmissionRate2 = calculateSupplyBasedEmissionRate(totalSupply, totalStaked, newTotalSupplyFactor);
      await time.increase(PARAM_UPDATE_DELAY.add(new BN(1)));
      expect(await DeStaking.getSupplyBasedEmissionRate()).to.be.bignumber.equal(supplyBasedEmissionRate2);
    ...
    it(`can't be more than 7.5%`, async () => {
      ...
      const totalSupply = ether('8537500');
      await stakeToken.mint(owner, totalSupply, { from: owner });
      await stakeToken.transfer(DeStaking.address, totalSupply.div(new BN(2)), { from: owner });
      expect(await DeStaking.getSupplyBasedEmissionRate()).to.be.bignumber.equal(maxSupplyBasedEmissionRate.div(new BN(2)));
      await DeStaking.setTotalSupplyFactor(ether('0.5'));
      await time.increase(PARAM_UPDATE_DELAY.add(new BN(1)));
      expect(await DeStaking.getSupplyBasedEmissionRate()).to.be.bignumber.equal(maxSupplyBasedEmissionRate);
      await stakeToken.transfer(DeStaking.address, totalSupply.div(new BN(2)), { from: owner });
      expect(await DeStaking.getSupplyBasedEmissionRate()).to.be.bignumber.equal(maxSupplyBasedEmissionRate);
    });
  });
  describe('getAccruedEmission', () => {
    it('should be calculated correctly', async () => {
      ...
      await stakeToken.mint(user1, value, { from: owner });
      await stakeToken.approve(DeStaking.address, value, { from: user1 });
      const receipt = await DeStaking.methods['deposit(uint256)'](value, { from: user1 });
      const timestampBefore = await getBlockTimestamp(receipt);
      await time.increase(YEAR.div(new BN(8)));
      ...
      const totalSupply = await stakeToken.totalSupply();
      const totalStaked = await DeStaking.totalStaked();
      const depositDate = await DeStaking.depositDates(user1, 1);
      const userAccruedEmission = calculateUserAccruedEmission(value, timePassed, totalSupply, totalStaked);
      expect((await DeStaking.getAccruedEmission(depositDate, value)).userShare).to.be.bignumber.equal(userAccruedEmission);
    });
  });
